...
begin
   ...
   if IniFile.ReadString('System','Version','') = '' then begin
     ...
     IniFile.WriteInteger('PCM1802','Smps', ADC_smps);
     IniFile.WriteInteger('PCM1802','Chnl', ADC_channel);
     IniFile.WriteFloat('PCM1802','Uk', Uk_adc);
     IniFile.WriteFloat('PCM1802','Uz', Uz_adc);
     IniFile.WriteFloat('PCM1802','Ik', Ik_adc);
     IniFile.WriteFloat('PCM1802','Iz', Iz_adc);
   end
   else begin
     ...
     ADC_smps := IniFile.ReadInteger('PCM1802','Smps', ADC_smps);
     ADC_channel := IniFile.ReadInteger('PCM1802','Chnl', ADC_channel);

     Uk_adc := IniFile.ReadFloat('PCM1802','Uk', Uk_adc);
     Uz_adc := IniFile.ReadFloat('PCM1802','Uz', Uz_adc);

     Ik_adc := IniFile.ReadFloat('PCM1802','Ik', Ik_adc);
     Iz_adc := IniFile.ReadFloat('PCM1802','Iz', Iz_adc);
...
begin
   ...
   buftx[4] := freq;  // freq
   buftx[5] := freq shr 8; // freq
   buftx[6] := freq shr 16; // freq
   buftx[7] := freq shr 24; // freq
...
begin
    ...
    if SetAdcIniCfg(1) then begin
        ...
        else if (ADC_channel = 2) then
            ChartEnables := CHART_I_MASK
        else
            ChartEnables := CHART_UI_MASK;
        s := ADC_smps;
        if s <> 0 then begin
          maxs := Round((SMP_BUF_CNT * 495.0)/s); // ms
